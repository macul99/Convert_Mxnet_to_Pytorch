...
class MyPytorchEmitter(Emitter):
    ...
    def convert(data):
        ...
        if isinstance(data, dict):   return dict(map(MyPytorchEmitter.convert, data.items()))
        if isinstance(data, tuple):  return map(MyPytorchEmitter.convert, data)
        return data

    def run(self, dstNetworkPath, dstWeightPath = None, phase = 'test'):
        super(MyPytorchEmitter, self).run(dstNetworkPath, dstWeightPath, phase)
    ...
    def parent_variable_name(self, IR_node, path=[0], weight_type='weights'):
        if not IR_node.in_edges and IR_node.name in self.weights_dict.keys():
            ...
            return "torch.from_numpy(self.weights_dict['{}']['{}'])".format(IR_node.name, weight_type)

        return super(MyPytorchEmitter, self).parent_variable_name(IR_node, path)
...
""")
    ...
    def emit_Conv(self, IR_node):
        ...
        self.add_init(2, "self.{} = self.__conv({}, weights_dict=self.weights_dict, name='{}', in_channels={}, out_channels={}, kernel_size={}, stride={}, groups={}, bias={})".format(
    ...
    def emit_FullyConnected(self, IR_node):
        ...
        self.add_init(2, "self.{} = self.__dense(name = '{}', weights_dict=self.weights_dict, in_features = {}, out_features = {}, bias = {})".format(
            IR_node.variable_name,
            IR_node.name,
            in_features,
    ...
    def emit_Embedding(self, IR_node):
        ...
        self.add_init(2, "self.{} = self.__embedding('{}', weights_dict=self.weights_dict, num_embeddings={}, embedding_dim={})".format(
            IR_node.variable_name,
            IR_node.name,
    ...
    def emit_Constant(self, IR_node):
        if IR_node.get_attr('value'):
            ...
            code = "self.{:<15} = torch.autograd.Variable(torch.from_numpy(self.weights_dict['{}']['value']), requires_grad=False)".format(
                IR_node.variable_name,
                IR_node.name)
        
        # self.add_init(2, "self.{:<15} = torch.from_numpy(self.weights_dict['{}']['value'])".format(
        #     IR_node.variable_name,
        #     IR_node.name))
    ...
    def emit_BatchNorm(self, IR_node):
        ...
        self.add_init(2, "self.{} = self.__batch_normalization({}, '{}', weights_dict=self.weights_dict, num_features={}, eps={}, momentum={})".format(
             IR_node.variable_name,
             dim,
             IR_node.name,
             num_features,
    ...
    def emit_Scale(self, IR_node):
        ...
        self.add_init(2, "self.{} = self.__scale({}, '{}', weights_dict=self.weights_dict, num_features={})".format(
             IR_node.variable_name,
             dim,
             IR_node.name,
    ...
    def emit_PRelu(self, IR_node):
        if self.ctx == 'cpu':
            code = "{:<15} = F.prelu({}, torch.from_numpy(self.weights_dict['{}']['weights']))".format(
                IR_node.variable_name,
                self.parent_variable_name(IR_node, [0]),
                IR_node.name)
        else:
            code = "{:<15} = F.prelu({}, torch.from_numpy(self.weights_dict['{}']['weights']).cuda())".format(
                IR_node.variable_name,
                self.parent_variable_name(IR_node, [0]),
                IR_node.name)
...
{}
        ...
        if not self.layers_codes.get(scope_node.pattern, None):
            ...
            for node_name in scope_node.topology_list:
                ...
                if hasattr(self, "emit_" + node_type):
                    ...
                    print("MyPytorchEmitter has not supported operator [%s]." % (node_type))
    ...
    def _layer_Embedding(self):
        self.add_body(0,"""
    @staticmethod
    def __embedding(name, weights_dict, **kwargs):
        ...
        def _check_input_dim(self, input):
            if input.dim() != 5:
                raise ValueError('expected 5D input (got {}D input)'
                                .format(input.dim()))
    @staticmethod
    def __scale(dim, name, weights_dict, **kwargs):
